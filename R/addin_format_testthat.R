...
prepare_test_that_file <- function(path) {
  ...
  test_that_desc_parsed <- str_extract(
  ...
  test_that_desc_cleaned <- str_remove(
  ...
  function_name <- fill(data.frame(name = function_name), name)$name
  ...
  test_that_lines_updated <- str_replace(
    string = test_that_lines,
    pattern = '(?<=test_that\\(").*"',
...
format_test_that_file <- function() {
  file_info <- getActiveDocumentContext()
  documentSave(id = file_info$id)
  result <- prepare_test_that_file(path = file_info$path)
  setDocumentContents(paste0(result$file_content, collapse = "\n"), id = file_info$id)
  documentSave(id = file_info$id)
}
